...
/**
 * Selon ton nouveau Prisma schema :
 *
 * Product
 *   -> paymentPageProducts
 *      -> paymentPage
...
const getToken = (): string | null => {
  ...
  const calculateStatistics = (
    ...
    return {
      ...
      published: productList.filter(
        (product) =>
          product.status === "PUBLISHED"
      ).length,

      draft: productList.filter(
        (product) =>
  ...
  const loadProducts = async () => {
    try {
      ...
      /**
       * Le backend ne renvoie pas statistics.
       * On les calcule donc ici.
       */
      setStatistics(
        calculateStatistics(productList)
      );
    } catch (err: any) {
      console.error(
        "GET PRODUCTS ERROR:",
  ...
  const publishProduct = async (
    ...
    try {
      ...
      const response = await fetch(
        `${getProductsUrl()}/${productId}/publish`,
        {
          method: "PATCH",
  ...
  const getPublicUrl = (
    ...
    const relations =
      product.paymentPageProducts || [];

    /**
     * On cherche d'abord une page active.
     */
    const activeRelation =
      relations.find(
        (relation) =>
          relation.paymentPage?.active
      );

    /**
     * Sinon on prend la première page disponible.
     */
    const relation =
      activeRelation ||
      relations[0];
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Gérez vos produits, formations,
              services et pages de paiement.
            </p>
          </div>
